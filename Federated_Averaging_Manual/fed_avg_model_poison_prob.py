...
def federated_avg(weights_list, aggregation_type, poison_probabilities):
    ...
    if aggregation_type == "mean":
        ...
        avg_weights = copy.deepcopy(weights_list[0])
        valid_weights = [weights_list[i] for i in range(len(weights_list)) if poison_probabilities[i] <= 0.3]

        print(f"Initial Weights: {len(weights_list)}")
        print(f"Valid Weights: {len(valid_weights)}")
        
        if not valid_weights:
            raise ValueError("No valid clients with probability <= 0.3 for aggregation.")
...
def train_client(global_model, data, client_id, epochs, num_model_poisoned_clients, scale_factor, is_model_poisoned):
...
def train_and_evaluate(num_clients, num_rounds, epochs, data, writer, num_data_poisoned_clients, num_model_poisoned_clients, scale_factor, aggregation_type, poison_probabilities, is_model_poisoned):
    ...
    for rnd in range(num_rounds):
        ...
        with ThreadPoolExecutor(max_workers=num_clients) as executor:
            futures = [
                executor.submit(train_client, global_model, data[client_id], client_id, epochs, num_model_poisoned_clients, scale_factor, is_model_poisoned)
...
if __name__ == "__main__":
    ...
    with open(csv_file, "w", newline="") as csvfile:
        ...
        for num_data_poisoned_clients in num_data_poisoned_clients_array:
            ...
            is_model_poisoned = rand_array < probabilities
            ...
            for client_id in range(num_clients):
                ...
                print(f"Client {client_id + 1} data poisoned: {is_model_poisoned[client_id]}")
