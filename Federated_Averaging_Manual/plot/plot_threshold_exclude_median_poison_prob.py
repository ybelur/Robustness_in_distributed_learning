...
file_paths = [
    f"../results/threshold_exclude_median_0.5_prob_model_poison_5_clients/run_{i:02d}.csv"
    for i in range(100)
]
...
for path in file_paths:
    df = pd.read_csv(path)
    run = int(os.path.basename(path).split('_')[1].split('.')[0]) + 1
    df['Trial Number'] = run
...
plt.title('Moving Average of Global Accuracy vs Rounds over several Trials (Threshold Exclude Median Model Poisoning)')
plt.xlabel('Round')
plt.ylabel('Global Accuracy (Moving Average)')
plt.grid(True)
plt.legend([], [], frameon=False) 
plt.savefig("../figures/threshold_exclude_median/5_clients/threshold_exclude_median_model_poison_100_prob.png", dpi=300)
